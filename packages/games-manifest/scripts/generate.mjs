...
function logicsSource() {
  ...
  const entries = games.map((g) => `  "${g.id}": ${g.logic},`).join("\n");
  return `// GENERATED by scripts/generate.mjs - do not hand-edit; run \`pnpm --filter @hubbub/games generate\`.
...
function settingsSource() {
  ...
  const entries = withSettings.map((g) => `  "${g.id}": ${g.settings},`).join("\n");
  return `// GENERATED by scripts/generate.mjs - do not hand-edit; run \`pnpm --filter @hubbub/games generate\`.
