...
- **Phase 3 - Local product (Electron host):** embed server, LAN-IP QR, installers. *Done = send a friend the app, play TTT on real WiFi.* First sendable product. **Superseded:** `apps/host-desktop` is parked as of 2026-08-05 and is no longer a priority or a gate on any later phase; the local workflow that matters is the terminal dev loop for game authors. See `2026-08-05-hubbub-cloud-hosting-and-game-distribution-design.md`.
