...
1. `packages/games-manifest/package.json` - add an **`optionalDependencies`** line
   (not `dependencies`; only the two in-repo core games live there, and a sibling
   repo has to be absent-tolerant so the platform still builds without it):
...
2. `packages/games-manifest/scripts/generate.mjs` - add ONE entry to the
   `EXTERNAL_GAMES` array, copying the `split-opinions` entry's shape:
```js
{
  id: "<gameId>",
  pkg: "@hubbub/game-<gameId>",
  dir: "hubbub-game-<gameId>",
...
```
   `screen`/`controller`/`logic` are the **exported symbol names** in the game repo,
   and the generator writes them into the import statements verbatim - a typo here
   surfaces as an unresolved import at typecheck, not as a missing game.

   **Never edit `src/logics.ts`, `src/lazy.ts` or `src/settings.ts`.** Since `6ef973c`
   those three are generated by this script on every `pnpm install`, and since `d6cdd87`
   they are gitignored, so an edit there cannot be committed and is wiped by the next
   install. The generator omits any sibling repo not checked out on disk, which is what
   lets the platform build without your game present.

   **Settings schema, only if the game has one:** add
   `"./settings": "./src/settings-schema.ts"` to the game's own `package.json` `exports`,
   then set `settings: "<GAME_ID_UPPER_SNAKE>_SETTINGS_SCHEMA"` (instead of `null`) in the
   entry above. `settings-schema.ts` must stay an import leaf - it is bundled eagerly into
   every browser app, so it must not import the game's `logic.ts` or anything else heavy.

   **What the tests actually catch, verified 2026-08-22 - do not over-trust this.**
   `manifest.test.ts` only asserts `GAME_CHUNKS`' keys match `GAME_LOGICS`' keys; both are
   generated from the same array, so it can no longer catch a missed registration at all.
   The only real guard is `settings-schema-leaf.test.ts`, which fails when a declared
   `@hubbub/game-*` dependency resolves a `/settings` subpath but is absent from the
   generated `settings.ts`. **A game with no settings schema has NO test coverage here** -
   forget the generator entry and it silently never appears. Confirm your game shows up in
   the lobby before believing a green run.
